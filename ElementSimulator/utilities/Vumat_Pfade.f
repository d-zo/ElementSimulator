program Stresspath_test
   ...
   do iangle = 1, num_angles
      inpstress(1, :) = pressure_origin
      outstress(1, :) = pressure_origin
      inpstate(1, :) = statevariables
      outstate(1, :) = statevariables
      ...
      loading_loop: &
      do
         if (any(outstress(1, 1:3) < minmax_stress(1)) .or. any(outstress(1, 1:3) > minmax_stress(2))) then
            exit loading_loop
         end if
