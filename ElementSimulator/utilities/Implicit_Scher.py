...
# Mesh
#
partElement.setElementType(elemTypes=(ElemType(elemCode=CPE4, elemLibrary=STANDARD),
   ElemType(elemCode=CPE3, elemLibrary=STANDARD)), regions=(partElement.sets['setElement']));
...
mymodel.DisplacementBC(amplitude=UNSET, createStepName='Initial', distributionType=UNIFORM,
   fieldName='', localCsys=None, name='bcUnterkanteHalten', region=instElement.sets['setUnterkante'],
...
mymodel.historyOutputRequests['AusgabeOberkante'].setValuesInStep(stepName=thisstep,
   numIntervals=100);
if (konstantes_volumen == 'ja'):
   mymodel.boundaryConditions['bcAbscheren'].setValuesInStep(
      stepName=thisstep, u1=max_scherung, u2=0.0);
else:
   mymodel.boundaryConditions['bcAbscheren'].setValuesInStep(
      stepName=thisstep, u1=max_scherung);
#
# Step: Laststufe
#
for izyklus in range(2, 2*zyklen + 2):
   ...
   mymodel.StaticStep(name=thisstep, timePeriod=steptime, initialInc=--timeincrement--, matrixSolver=DIRECT,
      matrixStorage=UNSYMMETRIC, maxNumInc=10000, noStop=OFF, previous=laststep,
      timeIncrementationMethod=AUTOMATIC);
   mymodel.steps[thisstep].Restart(overlay=ON, timeMarks=OFF);
   if (konstantes_volumen == 'ja'):
      mymodel.boundaryConditions['bcAbscheren'].setValuesInStep(stepName=thisstep,
         u1=(izyklus % 2)*max_scherung, u2=0.0);
   else:
      mymodel.boundaryConditions['bcAbscheren'].setValuesInStep(
         stepName=thisstep, u1=(izyklus % 2)*max_scherung);
#
# Job
#
